...
class EvaluationConfig:
    ...
    model_generated_outputs_path: str  #  the file to be evaluated # todo
    inputs_outputs_basepath: str  # todo # the basepath of the input and output files for evaluating each problem. unit tests are located in ${inputs_outputs_basepath}/${problem_id}/{input.txt, output.txt}
    ...
    # language
    language: str # programming language (e.g. python/cpp) # todo
    ...
    def get_args() -> argparse.ArgumentParser:
        ...
        args.add_argument(
            "--reference_file_path", 
            default = os.path.join(cg.DATA_PATH, f'{cg.LANG}_splits', f'test{cm.i_part}.jsonl'), #todo
            type=str
        )
        ...
        args.add_argument(
            ...
            type=str,
            # default= os.path.join(cg.DATA_PATH, f'{cm.LANG}_splits', f'results_{cm.LANG}_code.jsonl'), 
            #default= os.path.join(cg.DATA_PATH, 'output', f'{cm.model_id.split("/")[1]}_results_{cg.LANG}_{cm.num_input_examples}shot_{cm.num_return_sequences}samples_code.jsonl'), #todo
            default = os.path.join(cg.DATA_PATH, 'output', f'{cm.model_id.split("/")[1]}_results_{cg.LANG}_{cm.model_version}_{cm.num_input_examples}shot_{cm.num_return_sequences}samples{cm.i_part}{cm.llama2_round}_code.jsonl'), # todo
        ...
        args.add_argument(
            ...
            type=str, 
            default=cg.LANG,
            help="programmming language (e.g. 'python' or 'cpp')"
        )
        ...
        args.add_argument(
            ...
            type=str, 
            # default= os.path.join(cg.DATA_PATH, f'{cm.LANG}_splits', f'results_{cm.LANG}_code.jsonl.report'), 
            default = os.path.join(cg.DATA_PATH, 'output', f'{cm.model_id.split("/")[1]}_results_{cg.LANG}_{cm.model_version}_{cm.num_input_examples}shot_{cm.num_return_sequences}samples{cm.i_part}{cm.llama2_round}_code.jsonl.report'), # todo
        ...
        args.add_argument(
            ...
            default= os.path.join(cg.DATA_PATH, 'codenet', 'public_test_cases'), #todo
            type=str,
        )
        ...
        args.add_argument(
            "--temp_dir",
            type=str,
            default= os.path.join(cg.DATA_PATH, 'temp'), #todo
...
defaults = {
    ...
    'model_generated_outputs_path': None, 
    'model_generated_output_col': 'generated_answer', 
    'model_generated_potentially_faster_code_col': 'generated_answer', 
    'output_report_file_path': None, 
